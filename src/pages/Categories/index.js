...
const priceRanges = [
    ...
    { min: 700000, max: undefined },
];

const sortBy = [
    { name: 'Giá (tăng dần)', value: 'price', order: 'asc' },
    { name: 'Giá (giảm dần)', value: 'price', order: 'desc' },
    { name: 'Tên tác giả', value: 'author', order: 'desc' },
...
function Categories() {
    ...
    const bookListResponse = useGetBooks(
        { page, limit: pageSize },
        {
            minPrice: priceRanges[priceRangeId]?.min,
...
function BookList(props) {
    const { books, pageSize, currentPageNum, numOfPages, onChangePage } = props;
    return (
        <>
            <div className={cx('products-list')}>
                {books?.map((book, index) => {
                    return <Item key={index} {...book} />;
                })}
            </div>
            <div className={cx('content-right-footer')}>
                {new Array(numOfPages).fill(0).map((_, index) => (
                    <button
                        key={index}
                        className={cx('number-page', currentPageNum === index + 1 ? 'active' : '')}
                        onClick={() => {
